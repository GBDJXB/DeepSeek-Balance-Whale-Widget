...
> 当前版本：v0.2.10（含今日已用双模式、峰谷定价、随机台词、音效、汉堡菜单、每轮对话消耗统计与「距下次峰谷变价」倒计时提示）。
...
- **今日已用**双模式（菜单可选）：
  ...
  - 附带（气泡展示）：峰谷台词组第三行由重复的「今日已用 ¥X」改为静态「距下次峰谷变价约 X」（北京时间口径，打开时现算，详见「随机台词气泡」一节；「今日已用」仍显示在默认余额气泡）。
...
- 台词六组按权重随机（`pickRandomLines` 加权抽样）：
  1. 权重 20：三行（A 样式「当前时间段为:」/ B 样式峰谷「空闲时段」绿 或「高峰时段」红（P 档字号，比金额 B 略小）/ C 样式「距下次峰谷变价约 X」）
  2. 权重 7：居中 B「好模型... ↓」/「好女孩...↓」
...
5. 峰谷倒计时自测：点击气泡切到「当前时间段为:」组 → 第三行显示「距下次峰谷变价约 X」；按当前北京时间对照规则手算应吻合（如工作日 10:00 → 约2小时；工作日 13:00 → 约1小时；2026-08-23 后的周末任意时刻 → 至下周一 09:00 的时长）；默认余额气泡（余额 + 今日已用）不受影响、不出现该行。
