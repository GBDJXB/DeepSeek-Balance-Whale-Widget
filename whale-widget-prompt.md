...
- 台词六组按权重随机（`pickRandomLines` 加权抽样）：
  1. 权重 20：三行（A 样式「当前时间段为:」/ B 样式峰谷「空闲时段」绿 或「高峰时段」红（P 档字号，比金额 B 略小）/ C 样式「下次变价：X」）
  2. 权重 7：居中 B「好模型... ↓」/「好女孩...↓」
  ...
  6. 权重 1：居中 B「哦鲸鲸... 」
- 峰谷转换倒计时：组 1 的第三行显示距下一次 高峰/空闲 切换的剩余时间「下次变价：X」（如 `15小时`、`2天15小时`、`1小时30分`；≤1 分钟显示「峰谷价格将切换」）。
