...
var PEAK_HOURS = ${JSON.stringify(PEAK_HOURS)}
var WEEKEND_VALLEY_FROM_SEC = ${Number(WEEKEND_VALLEY_FROM_SEC)}
...
var BJ_MS = 8 * 3600 * 1000
function bjPeakAtMs(ms) {
  var n = Math.floor(ms / 1000)
  if (n >= WEEKEND_VALLEY_FROM_SEC) {
    var dow = new Date(ms + BJ_MS).getUTCDay() // 0=周日 6=周六
    if (dow === 0 || dow === 6) return false
  }
  var hour = new Date(ms + BJ_MS).getUTCHours()
...
function msUntilNextChange() {
  var step = 60000
  var maxSteps = 8 * 24 * 60 // 往后最多搜 8 天，覆盖周末全天谷价后的周一 09:00 变价
  var nowMs = Date.now()
  var cur = bjPeakAtMs(nowMs)
  var t = nowMs - (nowMs % step) + step
  for (var i = 0; i < maxSteps; i++) {
    if (bjPeakAtMs(t) !== cur) return t - nowMs
    t += step
  }
  return 0
}
function nextChangeText() {
  var ms = msUntilNextChange()
  if (ms <= 0 || ms <= 60000) return '峰谷价格即将切换'
  var totalMin = Math.round(ms / 60000)
  var parts = []
  var d = Math.floor(totalMin / 1440); if (d > 0) parts.push(d + '天')
  var h = Math.floor((totalMin % 1440) / 60); if (h > 0) parts.push(h + '小时')
  var m = totalMin % 60; if (m > 0 && parts.length < 2) parts.push(m + '分')
  return '下次变价：' + parts.join('')
}
...
function showPeakBubbleOnce() {
  if (!bubbleOn || bubbleShown) return
  showBubble()
  bubbleRandomActive = true
  bubbleRandomLines = buildGroup1()
  swapBubbleContent(function () { applyBubbleLines(bubbleRandomLines) })
}
function hideBubble() {
  if (bubbleTimer) { clearTimeout(bubbleTimer); bubbleTimer = null }
    ...
    .then(function (data) {
      if (data && data.ok) {
        ...
        if (!autoPeakShown) {
          autoPeakShown = true
          // 首次加载完成后自动展示一次峰谷气泡
          setTimeout(showPeakBubbleOnce, 800)
        }
      } else {
        state.status = 'error'
