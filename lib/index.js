...
// 每轮切换角色配置：存 4 个状态各自的形象（引用角色 id 数组，未来可扩展轮换/随机多形象）
// 与「是否在 N 秒后回到默认形象」的回退设置。独立文件，避免改动 size.json 的写接口。
const ROLE_SWITCH_FILE_CANDIDATES = [
  path.join(DSH_HOME, '.dshw-role-switch.json'),
  path.join(DSH_HOME, 'profiles', 'web', '.dshw-role-switch.json'),
]
...
export default {
  ...
  apply(ctx) {
    ...
    // —— 状态切换：6 个状态(默认/深度思考/输出/调用工具/调用工具失败/输出完成后)各自的形象与回退设置 ——
    // 形象引用「角色」模块里的角色 id('default'=内置小鲸鱼，其余为 whale-roles 的 id)。
    // 每个状态的 images 是数组，当前只取 [0]；未来「轮换/随机多形象」直接在此扩展取值策略。
    // minHold = 「最短持续时间」：进入某状态后至少停留多少秒才允许切到其它状态(防闪烁)，可全局或按状态。
    const ROLE_SWITCH_STATES = ['idle', 'thinking', 'output', 'tool', 'toolError', 'done']
    function roleSwitchMinHoldDefault() { return { on: false, sec: 0 } }
    function defaultRoleSwitchConfig() {
      const state = () => ({ images: [], revert: { on: false, sec: 0 } })
      const states = {}
      for (const k of ROLE_SWITCH_STATES) states[k] = state()
      const perState = {}
      for (const k of ROLE_SWITCH_STATES) perState[k] = roleSwitchMinHoldDefault()
      return { v: 2, enabled: false, minHold: { mode: 'global', global: roleSwitchMinHoldDefault(), perState }, states }
    }
    // 归一 {on, sec}：on 布尔化，sec 0–86400 整数；raw 缺字段时回退 base
    function normalizeRoleSwitchTimer(raw, base) {
      const r = raw && typeof raw === 'object' ? raw : {}
      const b = base && typeof base === 'object' ? base : {}
      const sec = Number(r.sec !== undefined ? r.sec : b.sec)
      return {
        on: typeof r.on === 'boolean' ? r.on : (b.on === true),
        sec: isFinite(sec) ? Math.min(86400, Math.max(0, Math.round(sec))) : 0,
      }
    }
    // base 传入时（PUT 场景）缺失的字段沿用 base 值，避免部分提交把其它状态清空
    function normalizeRoleSwitchConfig(raw, base) {
      const out = defaultRoleSwitchConfig()
      const r = raw && typeof raw === 'object' ? raw : {}
      const b = base && typeof base === 'object' && base.states ? base : null
      out.enabled = typeof r.enabled === 'boolean' ? r.enabled : (b ? b.enabled === true : false)
      const s = r.states && typeof r.states === 'object' ? r.states : {}
      for (const k of ROLE_SWITCH_STATES) {
        const st = s[k] && typeof s[k] === 'object' ? s[k] : {}
        const bs = b ? (b.states[k] || {}) : {}
        const imagesRaw = Array.isArray(st.images) ? st.images : (Array.isArray(bs.images) ? bs.images : [])
        const images = imagesRaw
          .map((x) => String(x))
          .filter((x) => x === 'default' || /^[A-Za-z0-9_-]{1,64}$/.test(x))
          .slice(0, 64)
        out.states[k] = {
          images,
          revert: normalizeRoleSwitchTimer(st.revert, bs.revert),
        }
      }
      // minHold：mode 只接受 global/perState；global/perState 各自归一
      const rMh = r.minHold && typeof r.minHold === 'object' ? r.minHold : {}
      const bMh = b && b.minHold && typeof b.minHold === 'object' ? b.minHold : null
      const modeRaw = rMh.mode !== undefined ? rMh.mode : (bMh ? bMh.mode : undefined)
      out.minHold.mode = modeRaw === 'perState' ? 'perState' : 'global'
      out.minHold.global = normalizeRoleSwitchTimer(rMh.global, bMh ? bMh.global : null)
      const rPs = rMh.perState && typeof rMh.perState === 'object' ? rMh.perState : {}
      const bPs = bMh && bMh.perState && typeof bMh.perState === 'object' ? bMh.perState : {}
      for (const k of ROLE_SWITCH_STATES) {
        out.minHold.perState[k] = normalizeRoleSwitchTimer(rPs[k], bPs[k])
      }
      return out
    }
    function readRoleSwitchConfig() {
      for (const p of ROLE_SWITCH_FILE_CANDIDATES) {
        try {
          const parsed = JSON.parse(fs.readFileSync(p, 'utf8'))
          if (parsed && typeof parsed === 'object') return normalizeRoleSwitchConfig(parsed)
        } catch (err) {}
      }
      return defaultRoleSwitchConfig()
    }
    function writeRoleSwitchConfig(cfg) {
      const normalized = normalizeRoleSwitchConfig(cfg)
      const body = JSON.stringify(normalized, null, 2)
      for (const p of ROLE_SWITCH_FILE_CANDIDATES) {
        try { fs.writeFileSync(p, body, 'utf8'); return normalized } catch (err) {}
      }
      return null
    }
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/role-switch.json',
      handler: async (req, res) => {
        try {
          if (req.method === 'POST' || req.method === 'PUT') {
            ...
            const parsed = JSON.parse(body || '{}')
            if (!parsed || typeof parsed !== 'object') throw new Error('bad body')
            const next = normalizeRoleSwitchConfig(parsed, readRoleSwitchConfig())
            const saved = writeRoleSwitchConfig(next)
          ...
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify({ ok: true, config: readRoleSwitchConfig() }))
        } catch (err) {
          res.writeHead(400, JSON_HEADERS)
          res.end(JSON.stringify({ ok: false, error: String((err && err.message) || err).slice(0, 200) }))
        }
